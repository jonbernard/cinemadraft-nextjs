...
| D46 | **There is no live draft in Cinemadraft, so Phase 6 does not build one.** Verified in the source before building: no clock, no current-pick pointer, no turn timer, and no column in `drafts` or `draft_picks` that could express one. What exists is the **league owner entering picks on behalf of seats**, grouped (league 1's 2026 season is 4 groups × 4 seats), with `draft_picks.order` reorderable by drag. Spec §6.7's "cell on the clock" is therefore a **new feature**, and D14 puts new features after cutover — building it now would mean inventing a data model mid-migration with nothing for the Phase 7 parity audit to compare against. Moved to Phase 15. The §6.7 board redesign that does *not* depend on a clock — seats down, rounds across, poster thumbnails so a taken film is unmistakable from artwork — still ships in Phase 6, and that is what the phase gate measures |
