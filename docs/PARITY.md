...
| Subscribe to ceremony dates as a calendar | **ported** | `GET /events/calendar.ics` | `app/api/ical/[...slug]/route.ts` (hand-rolled RFC 5545, no dependency added) + `lib/services/ical.ts` — one of D8's three `/api` routes. `[...slug]` scopes to one show's abbreviation; the un-slugged form still serves every show. Subscribe URL surfaced on `/award-shows` via `InviteLink`, a copyable field rather than a download link | ✓ |
...
| Capability | Verdict | Source | Port / task | Data |
|---|---|---|---|---|
| Rules and scoring explained | **ported** | `/rules-and-scoring` — static copy | `app/(app)/how-it-works/page.tsx` — nav entry's `ready` flag flipped now the page exists. 🔴 Corrects the source's "a nomination and a win are worth the same value": `lib/services/scoring.ts` doubles a win, so the page says that instead | n/a Route renamed by P18.T0; the old URL 308s. |
| The scoring rulebook by tier | **ported** | `GET /points` | `app/(app)/how-it-works/page.tsx`, same surface as T46 → `lib/services/scoring-table.ts` groups `pointRepository.findAll()` by level (descending value) then tier (ascending) — a rendering decision the repository's own doc comment says belongs to its caller, not the repository | ✓ Route renamed by P18.T0; the old URL 308s. |
