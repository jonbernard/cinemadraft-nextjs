...
- T2: `/api/live/[abbr]/stream` — the SSE route, closing itself at ~290s ahead of the platform's 300s kill. 🔴 Land it **after P17.T16b**: `leagues` is `[]` in T16a and widens in T16b.
  ...
  🔴 **The route needs its own `isPublic` entry, and this is not obvious.** P17.T16 made `/live/(.*)` public, but that pattern does **not** match `/api/live/…`, and `config.matcher` explicitly includes `/(api|trpc)(.*)` — so on a page a stranger can read, that stranger's browser would be bounced the moment `EventSource` opened the stream, and the page would sit there never updating. Found by T16b's implementer while reading the transport decision, not by a test: no spec can catch it, because under `E2E_TEST_AUTH=1` the proxy is a pass-through with no route protection at all. `proxy.test.ts`'s verbatim list is the only guard. Add the entry, pin it there, and decide deliberately what an anonymous reader's stream is allowed to contain — the page already proves the signed-out view leaks no seat names, league name or scores, and the stream must hold that same line
