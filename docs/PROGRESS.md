...
- [x] P17.T8 — a11y batch: 17 unnamed poster links, skip link, focus-ring token on the rail and the theme toggle
...
- [x] P18.T1 — `/how-it-works` replaces `/rules-and-scoring`, content intact — `65219f7`. A **move, not a rewrite**: every word of the old prose and its table landed on the new route unchanged, so the product was never worse than what it replaced at any commit. 🔴 The redirect is in `next.config.ts`, not a surviving page or `proxy.ts`, because `redirects` runs **before** Proxy — a year of league chat's links resolve without auth seeing them and without a function invocation, which is also why the old route no longer needs an `isPublic` entry. Verified against a production build: 308 + `location: /how-it-works` on the old route, 200 on the new. Two things the plan missed: the route directory also held `page.test.tsx`, and Biome's `useUniqueElementIds` rejects the spine's static section ids (kept, with the suppression `AppShell` already uses for the skip link — they are shareable fragments)
- [x] P18.T2 — the worked example, computed and never transcribed — `991396d`, `01c93e2`. `lib/services/how-it-works.ts` walks seasons newest-first for a board with rows; the component prints `MovieLedger.total` and derives each line's multiplier as `earned / points`, so **no numeric literal in the file describes the game**. The real example is **One Battle After Another, 620 across 50 lines**; the casualty is **War of the Worlds, −185**. 🔴 Fifty rows is true and unreadable in a beat, so `limit` shows the largest contributors and collapses the rest into one row whose value is **`total` minus what is displayed** — defined as the difference, the visible column adds up by construction and cannot drift. Ranked by magnitude, because on a negative ledger ranking by value surfaces the least damaging lines. 🔴 The plan's `Promise.all([getActiveYear(), availableSeasons()])` 500s a public page on a fresh database: `getActiveYear` throws on an empty `available_years`
- [x] P18.T3 — the rulebook, grouped by show with the tiers named — `c22e96f`, plus the marks and the two-column fix. A definition list, not a three-column grid: the old table put the tier meanings three paragraphs above the numbers and scrolled sideways at 390px. 🔴 **It also names a penalty level for the prize it hands out** — against the real table the generic labels printed "Best Picture −20" under the Razzies, a category nobody is nominated for, reading as a scoring bug rather than the joke. Keyed off the values, never the level's name. 🔴 A level with one or two marks sets them **beside** its figures (`grid-template-columns: 64px 1056px` at 1440, measured); three or more stack. That last fix shipped a commit late — `0a18539` described an edit that had silently not applied, and the owner caught it in the rendered page
- [x] P18.T4 — the twelve shows — built as `ShowsWall` (`765e7b5`), then **folded into the rulebook and the component deleted**. A wall of twelve marks followed by the same four groups as figures is one idea read twice; beside its own values, a mark answers the question a reader actually has. 🔴 The join is **awards, not events**: an event carries no level, `awards.points` is a foreign key into `points.id` (D41), and `points.level` is the group — `getShowGroups()` owns it, with a test proving a show whose only category references no point row joins nothing. Measured: Oscars 20/15/10, Globes 15/10/5, nine Alphabet shows flat at 5, Razzies −20/−15/−10
- [x] P18.T5 — the season shape — built as `SeasonShape`/`SeasonBeat`/`SeasonPhaseBeats` (`2e6714d`) and **superseded on the page by the landing-page rebuild**, which states the season's length as one derived sentence instead. The durable half is the extraction: `toSeasonPhases` / `getSeasonPhases` moved out of `getDashboard` into `lib/services/season.ts`, so the dashboard and any public surface build "one box per scoring moment" from **one definition** rather than two that can drift
- [x] P18.T6 — **verification, not code**, and this is the measurement (`7f2f151`). Production build, signed out, 1440×900: `--color-brass-fill` `#cfa93a`, the "Won" chips render `rgb(207, 169, 58)` — the same colour — 8 of them on the page, and **0 running animations** under `prefers-reduced-motion: no-preference` *and* `reduce`. So brass reaches a logged-out reader on an award outcome and nothing else (D99), which is what P17.T21 asked this task to prove. 🔴 The page ships no bespoke animation **by decision**: inventing motion for a prose page so there is something to attach a reduced-motion path to is decoration with a compliance justification, and the falsifiable form of the requirement is the zero above
- [x] P18.T7 — the way in — `7f2f151`. A closing band in display type carrying the pitch's last honest line and the primary action for the **second and last** time; the hero has the first. The returning-member sentence lives beside the register action rather than in a footer
- [x] P18.T8 — metadata, canonical and a share card — `ce2e726`. Static `metadata`, not `generateMetadata`: every field is a constant, so a function could only be wrong. The card carries the mark, the pitch and the wordmark and **no numbers** — a cached PNG cannot be re-derived. Verified against a production build with **real Clerk middleware**, and the PNG was opened and looked at (Satori fails silently). 🔴 **The card needs its own `proxy.ts` entry, as a pattern**: `isPublic` matches exact paths and Next emits the route at `/how-it-works/opengraph-image-<hash>`, where the hash moves whenever the card's code does. Counterfactual observed — before the entry, the exact card URL answered `307 → /auth/login`
- [x] P18.T9 — the gate — `b4b0db0`. Nine specs, **no `skipWithoutRestoredCorpus` anywhere**: each asks the database what it holds and requires the page to show exactly that, so the same assertions run on the restored copy and on CI's empty one (8 pass, 1 honestly skips). 🔴 The scoring assertions derive their expected values from **SQL that states the rule independently** — points resolved through the `awards.points` FK, doubled on a winner, inner-joined so an unpriced award earns no line — rather than reading a number off the page and comparing it to itself. Mutating `scoring.ts`'s doubling to `× 3` turns it red. 🔴 The plan declared the empty-season case untestable in a browser and substituted a unit test; it is testable, because the expectation can be computed from the database rather than assumed
- [x] P18.T10 — the signed-out home leads with the pitch — `d1cb06c`. Hero at the top: headline, the pitch with its Razzie clause, "Start a league", a quiet link to `/how-it-works`, the returning-member sentence beneath the action, and **the season's poster wall** — 8 real films, 326px tall at 1440, with the season rail still on the fold beneath it (D44 intact). The duplicate "Play the season" block at the bottom is deleted, and the comment arguing for it went with it. 🔴 **One pitch, one place**: `lib/copy.ts` owns the headline and the sentence and both public surfaces import them — two hand-typed copies drift the first time either page is edited. Signed-in `/` is unchanged, and `getLandingFacts` is **not called at all** when signed in, pinned by a test. 🔴 Noticed and left: `/live/[abbr]` carries the same duplicated pitch block

### Phase 18 notes

- **Phase 18 is complete (2026-09-12), CI-shape verify green** — 1,362 unit
  tests and 96 e2e against a fresh migrations+seed database, no scratch rows
  left. Decisions D105–D108. Both worktrees torn down, both branches deleted
  with `-d`.

- 🔴 **The page was rebuilt twice, both times because the owner looked at it.**
  The first build followed the locked direction (a season spine) and was
  correct and wrong: it put a landing page in the app's own register — one
  column, one rhythm, 4,394px — and read as documentation. The second cut the
  copy to a third, moved to bands, and added a display type step (D105). The
  lesson is not "ask more" but "render earlier": every defect that mattered was
  visible in a screenshot and invisible in the tests, including a typed
  "September to March" contradicting a derived "about 3 months" three
  centimetres above it.

- 🔴 **Two commits described changes that had not happened.** `0a18539` claimed
  the two-column rulebook fix and `262008c` left lint red on `main`. Both were
  silent no-match edits, committed without re-reading the file. The rule that
  would have caught them is the project's own: measure the result, do not
  assert it — the fix is now recorded as `grid-template-columns: 64px 1056px`,
  read out of the browser.

- **Numbers the phase measured, all from the restored corpus:** the season's
  best team pick is One Battle After Another at **620** across 50 ledger lines;
  the worst is War of the Worlds at **−185**; **12** shows score, **129** films
  scored in 2026, **10** seasons have been played. Point values: Oscars
  20/15/10, Globes 15/10/5, Alphabet flat 5, Razzies −20/−15/−10.
...
- [x] P14.T3 — the stream — `e91712c`. `/api/live/[abbr]/stream`, Node runtime, SSE. Polls every 2s and writes only when the serialisation differs; ten quiet polls send a comment heartbeat; self-closes at 290s ahead of the platform's 300s kill, clearing every timer on abort, on `cancel()` and on that close. A poll that throws is a skipped beat, not an error path. See **D110** for the frame contract and the 204. 🔴 The plan's instruction to pin the route in `proxy.test.ts` was already dead — the Clerk migration moved that mechanism to `test/route-protection.ts`, whose enumeration test went red demanding a gate until the route was listed, which is the mechanism working
- [x] P14.T4 — the client, its reconnect, and its stop conditions — `c792554`. `LiveRoom`, a client component holding the grid and the rosters; the header stays server-rendered because nothing in it moves during a ceremony. The server's frame is the initial value, so the first paint is still the server's and a crawler sees the whole room. **Measured, not estimated**: a visible idle tab opens **3 connections in 10 minutes** (gaps of 293.0s — the route's 290s close plus the browser's 3s retry); a hidden tab opens **0**, which is falsifiable because a connection left open would have reconnected twice in that window. A winner reached a second window **1,295 ms** after the row was written, with no navigation. 🔴 Eight mutations were run and **one survived**: dropping the `document.hidden` check at mount left a tab that starts hidden — a restored browser, a background-opened tab — holding a stream nobody is looking at, with no `visibilitychange` ever to close it. A test was added, the mutation then went red
