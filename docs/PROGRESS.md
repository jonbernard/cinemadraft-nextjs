...
- [x] P14.T3 — the stream — `e91712c`. `/api/live/[abbr]/stream`, Node runtime, SSE. Polls every 2s and writes only when the serialisation differs; ten quiet polls send a comment heartbeat; self-closes ten seconds short of the platform's per-plan ceiling (**D115** — Hobby's is 60s, not the 300s the spec assumed), clearing every timer on abort, on `cancel()` and on that close. A poll that throws is a skipped beat, not an error path. See **D110** for the frame contract and the 204. 🔴 The plan's instruction to pin the route in `proxy.test.ts` was already dead — the Clerk migration moved that mechanism to `test/route-protection.ts`, whose enumeration test went red demanding a gate until the route was listed, which is the mechanism working
