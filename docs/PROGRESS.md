...
- [ ] P18.T10 — 🔴 **Signed-out `/` leads with the pitch instead of ending with it.** Owner's call 2026-09-12, looking at the live page: the `EmptyState` headed "Play the season" sits at the **bottom** of the signed-out dashboard and repeats the one-line lede at the top of the same page, so a stranger reads the same argument twice and the second time is below a season rail they have no context for. Three parts: (1) a real hero at the top — headline, the one-line pitch **including the Razzie clause**, a primary "Start a league", and a quiet link into `/how-it-works`; the season rail stays directly beneath it, because a live season is the product's own evidence and the strongest thing on the page (D44). (2) Delete the bottom block. Its one non-duplicate sentence is the returning-member reassurance ("Register with the same email and your leagues, drafts and points come with you") — 🔴 that is real and must not be lost: it moves beside the register action, where somebody is deciding, not into a footer. (3) 🔴 **Derive, do not copy** — the hero and the `/how-it-works` opener render from one component and one string, or they drift the first time either is edited. **Runs after P18.T7**, which settles the wording. Note the existing code comment at `app/(app)/page.tsx` argues for the bottom block deliberately ("the returning member's reassurance… belongs at the end of a page somebody has read"); that reasoning is superseded — it was written when there was no explainer page to carry it, and the comment goes with the block
