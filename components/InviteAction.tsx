...
export function InviteAction({ url, className }: { url: string; className?: string }) {
  return (
    <details className={cn('w-fit', className)}>
      ...
      <summary className="border-border-rule text-text-primary hover:bg-bg-surface focus-visible:outline-accent-fill box-border flex min-h-11 w-fit cursor-pointer list-none items-center rounded-sm border px-4 text-sm focus-visible:outline-2">
        Invite
      </summary>
